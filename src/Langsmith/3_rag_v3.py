...
PDF_PATH = "src\Langsmith\islr.pdf"  # <- change to your file

# ----------------- helpers (not traced individually) -----------------
@traceable(name="load_pdf", tags=['Pdf', 'load'], metadata={"loader_cls": "PyPDFLoader"})
def load_pdf(path: str):
    loader = PyPDFLoader(path)
    return loader.load()  # list[Document]

@traceable(name="split_documents", tags=['split'], metadata={"splitter_cls": "RecursiveCharacterTextSplitter"})
...
@traceable(name="build_vectorstore", tags=['mbeddings', 'vectorstore'], metadata={"embedding_cls": "OpenAIEmbeddings", "embedding_model":"text-embedding-3-small", "vector_store":"FAISS"})
